...
export function createFinanceUI({api,getState,can,page,heading,metric,empty,field,input,option,esc,money,value,cents,dateLabel,today,showModal,modal,icon}) {
  ...
  function categorySelect(name,selected='',legacy='',kind='') {
    return `<select name="${name}">${option('','Sem categoria',selected||(!legacy?'':'legacy'))}${!selected&&legacy?option('legacy',`${legacy} (registro anterior)`,'legacy'):''}${categories().filter(c=>(c.active&&(!kind||accepts(c.applicability,kind)))||c.id===selected).map(c=>option(c.id,`${c.name}${c.active?'':' (inativa)'}`,selected)).join('')}</select>`;
  }
  ...
  function classificationFields(kind,categoryId='',legacy='',subId='') {
    ...
    return `<div class="expense-classification full" data-expense-classification>${field('Categoria (opcional)',categorySelect(prefix+'category_id',categoryId,legacy,type))}${field('Subcategoria (opcional)',`<select name="${prefix}subcategory_id" ${!categoryId?'disabled':''}>${option('','Sem subcategoria',subId)}${children.map(c=>option(c.id,c.name+(c.active?'':' (inativa)'),subId)).join('')}</select><small>${categoryId?'Selecione um detalhamento da categoria.':'Escolha uma categoria primeiro.'}</small>`)}</div>`;
  }
  function refreshClassification(element) {
    ...
    const kind=bulk?'variable':form.querySelector('[name="kind"]').value;
    ...
    target.outerHTML=classificationFields(bulk?'bulk':kind,categoryId==='legacy'?'':categoryId,categoryId==='legacy'?old?.category??'':'',subId);
  ...
  function expensePeriodForm(id,month) {
    const current=today().slice(0,7),preset=!month?'all':month===current?'current':'month';
    return `<form id="${id}" class="finance-period period-picker">${field('Período',`<select name="period" data-expense-period aria-label="Período das despesas">${option('all','Todos os meses',preset)}${option('current','Este mês',preset)}${option('month','Escolher mês',preset)}</select>`)}<div class="period-month" ${preset==='month'?'':'hidden'}>${field('Mês de referência',input('month',month||current,`type="month" min="2000-01" max="2199-12" required ${preset==='month'?'':'disabled'}`))}</div><button type="submit">Consultar</button><p class="period-help">Mês em que a despesa entra no resultado da loja, independentemente do dia do pagamento.</p></form>`;
  }
  ...
  function expenseSearchForm(current,formId='fin-expense-list-filter') {
    return `<form id="${formId}" class="finance-search">${input('search',current.search,'type="search" placeholder="Buscar descrição, categoria ou favorecido" aria-label="Buscar despesas"')}${categoryFilter(current.category)}<select name="status" aria-label="Situação da despesa">${[['','Todas as situações'],['pending','A pagar (todas)'],['overdue','Atrasadas'],['today','Vencem hoje'],['paid','Pagas'],['cancelled','Canceladas']].map(([v,l])=>option(v,l,current.status)).join('')}</select><button type="submit">Filtrar</button>${button('list-clear','Limpar filtros','','small subtle')}</form>`;
  }
  ...
  function expenseTypeCard(type,s,total) {
    ...
    return `<section class="panel expense-type-card"><div class="panel-header"><h2>${icon(type==='fixed'?'clock':'money')} ${label}</h2><span class="badge">${s.count} ${s.count===1?'conta':'contas'}</span></div><div class="panel-body"><div class="expense-type-total"><strong>${money(s.total)}</strong><span>${share(s.total,total)} das despesas</span></div><div class="expense-type-details"><div><span>Pago</span><strong>${money(s.paid)}</strong></div><div><span>A pagar</span><strong>${money(s.open)}</strong></div><div><span>Em atraso</span><strong>${money(s.overdue)}</strong></div></div><button type="button" class="small" data-action="fin-drill-type" data-id="${type}">Ver contas ${label.toLocaleLowerCase('pt-BR')} ${icon('arrow')}</button></div></section>`;
  }
  ...
  function expenseListPage(section) {
    ...
    const rows=inPeriod.filter(e=>matchesCategory(e,f.category)&&(!f.status||e.state===f.status||(f.status==='pending'&&!e.paid_date&&!e.voided_at))&&(!f.search||`${e.description} ${e.payee} ${e.category} ${e.subcategory??''}`.toLocaleLowerCase('pt-BR').includes(f.search.toLocaleLowerCase('pt-BR'))));
    const total=predicate=>rows.filter(e=>!e.voided_at&&predicate(e)).reduce((s,e)=>s+e.amount_cents,0);
    page(heading(`Despesas ${type}`,`Todas as contas ${type} cadastradas, com histórico de pagamentos.`,can('expenses.manage')?`<div class="actions">${button('new',`${icon('plus')} Nova despesa`)}${section==='variable'?button('bulk',`${icon('plus')} Cadastrar várias`,'','primary'):''}</div>`:'')+
      expensePeriodForm('fin-expense-list-period',f.month)+`<div class="metric-grid">${metric(`Total de ${type}`,money(total(()=>true)),'Sem contas canceladas','money')}${metric('Já pago',money(total(e=>!!e.paid_date)),'No período selecionado','check')}${metric('Falta pagar',money(total(e=>!e.paid_date)),'Contas em aberto','clock',true)}${metric('Em atraso',money(total(e=>e.state==='overdue')),'Vencidas e não pagas','clock')}</div><section class="panel"><div class="panel-header"><div><h2>Lista de despesas ${type}</h2><p>O mês de referência aparece junto do vencimento. Meses fechados mantêm seus valores protegidos.</p></div><span class="badge">${rows.length} registros</span></div><div class="expense-toolbar">${expenseSearchForm(f,'fin-expense-list-filter')}</div>${expenseTable(rows)}</section>`);
  }
  ...
  function bulkRow(month) {
    return `<fieldset class="expense-bulk-row" data-bulk-row><legend>Despesa <span data-bulk-number></span></legend><div class="expense-bulk-fields">${field('Descrição',input('bulk_description','','required maxlength="200" placeholder="Ex.: material de limpeza"'))}${field('Valor (R$)',input('bulk_amount','','required inputmode="decimal" placeholder="0,00"'))}${field('Vencimento',input('bulk_due_date','','type="date" required min="2000-01-01" max="2199-12-31"'))}${field('Mês de referência',input('bulk_reference_month',month,'type="month" required min="2000-01" max="2199-12"'))}${button('bulk-remove',icon('x'),'','small subtle danger')}</div><details class="bulk-extra"><summary>Categoria, subcategoria e observações (opcionais)</summary><div class="fields">${classificationFields('bulk')}${field('Favorecido / fornecedor',input('bulk_payee','','maxlength="150"'))}${field('Observações',input('bulk_notes','','maxlength="2000"'),true)}</div></details></fieldset>`;
  }
  ...
  function openBulk() {
    modalDirty=false;
    const month=listFilters.variable.month||expenseMonth;
    showModal('Cadastrar várias despesas variáveis',`<p class="finance-explainer">Preencha uma conta por linha. Cada despesa terá seu próprio valor, vencimento e mês de referência.</p><div class="bulk-defaults">${field('Lembrar quantos dias antes? (todas as linhas)',input('reminder_days',3,'type="number" required min="0" max="30"'))}</div><div id="expense-bulk-rows">${bulkRow(month)}</div><div class="bulk-footer">${button('bulk-add',`${icon('plus')} Adicionar despesa`)}<strong id="expense-bulk-summary" role="status" aria-live="polite"></strong></div><p class="footer-note">Até 50 despesas por cadastro. Todas serão salvas como variáveis e a pagar, sem repetição mensal. Se alguma linha estiver incorreta, nenhuma será salva.</p>`,'fin-bulk-form',`data-request-id="${crypto.randomUUID()}" data-default-month="${month}"`);
  ...
  return {
    ...
    dirty:()=>modalDirty,
    resetModal(){modalDirty=false;modal.classList.remove('expense-bulk-modal');},
    requestClose() {
    ...
    async submit(form,d) {
      ...
      if(form.id==='fin-bulk-form') {
        ...
        const expenses=rows.map((row,i)=>{
          ...
          try {if(!v('description').trim()||!v('amount').trim()||!v('due_date')||!v('reference_month'))throw Error('Preencha descrição, valor, vencimento e mês de referência.');const amount=cents(v('amount'));if(amount<=0)throw Error('O valor deve ser maior que zero.');return {description:v('description'),amount_cents:amount,due_date:v('due_date'),reference_month:v('reference_month'),category_id:v('category_id')||null,subcategory_id:v('subcategory_id')||null,payee:v('payee'),notes:v('notes'),kind:'variable',reminder_days:Number(d.reminder_days)};}catch(error){throw Error(`Linha ${i+1}: ${error.message}`);}
        });
