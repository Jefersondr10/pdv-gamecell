...
export function refundManagement(sale,{esc,money,date,can}) {
 ...
 const guidance=pending>0
  ? '<p class="refund-guidance">Depois de devolver o dinheiro ao cliente por fora do sistema, registre aqui para retirar a pendência. O sistema não faz Pix nem estorno automaticamente.</p>'
  : '<p class="refund-guidance refund-complete-message">Todo o valor recebido neste pedido já foi marcado como devolvido.</p>';
 ...
 return `<section class="panel refund-management" aria-label="Devolução ao cliente"><div class="panel-header"><div><h2>Devolução ao cliente</h2><p>Situação do valor recebido no pedido cancelado.</p></div><span class="badge ${state==='completed'?'good':'warn'}">${esc(refundLabels[state]??'Pendente')}</span></div><div class="panel-body"><div class="refund-summary"><div><span>Recebido</span><strong>${money(original)}</strong></div><div><span>Já devolvido</span><strong>${money(completed)}</strong></div><div class="${pending>0?'refund-due':'refund-done'}"><span>Falta devolver</span><strong>${money(pending)}</strong></div></div>${guidance}${action}${records}</div></section>`;
}
