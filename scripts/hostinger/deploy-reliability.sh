...
case "$source_dir" in /opt/atacadoapple/release-*) ;; *) exit 2;; esac
...
docker exec atacadoapple-app node --input-type=module -e 'import fs from "node:fs"; const s=JSON.parse(fs.readFileSync("/data/backups/last-success.json")); if(Date.now()-(s.completedAt??s.capturedAt)>300000)process.exit(1);'
cp -p .env ".env.pre-$release"
cp -p compose.yaml "compose.yaml.pre-$release"
systemctl stop atacadoapple-backup.timer
trap 'systemctl start atacadoapple-backup.timer' EXIT
rollback() {
  ...
  echo 'Application rolled back. Additive table, production records and files retained.'
  exit 1
}
