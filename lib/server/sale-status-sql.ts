...
export const SALE_ISSUE_SQL: Record<SaleIssueKey, string> = {
  ...
  review: `EXISTS (${receipts} AND ${receiptRequiresReviewSql('ar')})`,
  reading: `EXISTS (${receipts} AND EXISTS (SELECT 1 FROM receipt_ocr_jobs j WHERE j.attachment_id = ar.id AND j.status IN ('pending', 'processing', 'retry')))`,
...
export const SALE_AUTO_STATUS_SQL = SALE_CHECK_STATUS_SQL;
