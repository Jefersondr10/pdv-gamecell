...
const incompleteReading = `(a.receipt_details_json IS NULL OR NOT json_valid(a.receipt_details_json) OR
  (CASE WHEN json_valid(a.receipt_details_json) THEN
    ...
    (COALESCE(json_extract(a.receipt_details_json,'$.state'),'unknown')='unknown'
      OR COALESCE(json_extract(a.receipt_details_json,'$.automaticEligible'),0)=0
      OR json_extract(a.receipt_details_json,'$.ambiguous')=1
      OR NULLIF(TRIM(COALESCE(json_extract(a.receipt_details_json,'$.recipientBank'),'')),'') IS NULL)
  ELSE 0 END))`;
...
export async function processReceiptJob(db, files, engineUrl, now = Date.now) {
  ...
  try {
    ...
    const accepted =
      ...
      Boolean(receiptEvidenceKey(details)) &&
