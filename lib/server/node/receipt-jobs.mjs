...
// Explicit manual corrections and cancelled/blocked documents stay untouched.
...
export async function processReceiptJob(db, files, engineUrl, now = Date.now) {
  ...
  await db
    ...
    WHERE a.kind='receipt' AND a.receipt_amount_source IS NOT 'manual' AND s.status='completed'
  ...
  await db
    ...
    WHERE attachment_id IN (SELECT j.attachment_id FROM receipt_ocr_jobs j
      ...
      AND s.status='completed' AND a.receipt_amount_source IS NOT 'manual' AND ${incompleteReading}
  ...
  await db
    ...
    SELECT 1 FROM attachments a JOIN sales s ON s.id=a.sale_id AND s.store_id=a.store_id WHERE a.id=attachment_id AND (a.receipt_amount_source='manual' OR s.status!='completed'))`)
    .bind(timestamp)
    .run();
  ...
  try {
    const attachment = await db
      .prepare(
        `SELECT a.r2_key AS key, a.mime_type AS mimeType, a.size_bytes AS sizeBytes,
        a.receipt_amount_cents AS previousAmount,a.receipt_amount_confirmed_at AS previousConfirmedAt,
        a.receipt_details_json AS previousDetails,a.receipt_review_reason AS previousReview
        FROM attachments a JOIN sales s ON s.id=a.sale_id AND s.store_id=a.store_id WHERE a.id=? AND a.kind='receipt' AND a.receipt_amount_source IS NOT 'manual' AND s.status='completed'`,
      )
      .bind(claimed.id)
      .first();
    ...
    await db.batch([
      db
        ...
        WHERE id=? AND kind='receipt' AND receipt_amount_source IS NOT 'manual' AND receipt_amount_cents IS ? AND receipt_amount_confirmed_at IS ?
      ...
      db
        ...
        FROM attachments a WHERE a.id=? AND a.receipt_amount_confirmed_at=? AND a.receipt_amount_source IS NOT 'manual'
        ...
        .bind(
          `receipt-reading:${claimed.id}:${claimed.generation}`,
          JSON.stringify({
            ...
            after: { amountCents: amount, details },
          }),
          finished,
          claimed.id,
          finished,
      ...
      // OCR completion must wake settlement even if an earlier reading ended in
      // review. Explicitly preserved/manual settlement stays untouched.
      db
        ...
        WHERE a.id=? AND a.receipt_amount_confirmed_at=? AND a.receipt_amount_source IS NOT 'manual'
        AND q.status IS NOT 'manual'
        AND EXISTS(SELECT 1 FROM receipt_ocr_jobs j WHERE j.attachment_id=a.id AND j.status='processing' AND j.generation=? AND j.lease_token=?)
      ...
      db
        .prepare(`UPDATE receipt_ocr_jobs SET status=CASE WHEN EXISTS(SELECT 1 FROM attachments a WHERE a.id=attachment_id AND a.receipt_amount_source='ocr' AND a.receipt_amount_confirmed_at=?) THEN ?
        WHEN EXISTS(SELECT 1 FROM attachments a JOIN sales s ON s.id=a.sale_id WHERE a.id=attachment_id AND (a.receipt_amount_source='manual' OR s.status!='completed')) THEN 'cancelled' ELSE 'needs_review' END,
