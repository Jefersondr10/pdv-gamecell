...
export async function processReceiptJob(db, files, engineUrl, now = Date.now) {
  ...
  try {
    ...
    await db.batch([
      db
        .prepare(`UPDATE attachments SET receipt_amount_cents=?, receipt_amount_source='ocr', receipt_amount_confirmed_by=NULL, receipt_amount_confirmed_at=?
        WHERE id=? AND kind='receipt' AND receipt_amount_cents IS NULL AND ? IS NOT NULL
        ...
        .bind(amount, finished, claimed.id, amount, claimed.generation, token),
      db
