...
export async function processReceiptJob(db, files, engineUrl, now = Date.now) {
  const timestamp = now();
  // Bounded catch-up gives every historical receipt one pass through the
  // current reader. This includes old false positives/negatives and jobs whose
  // legacy reader_revision is NULL; terminal rows reopen at most once for each
  // revision, so a permanently unreadable file cannot loop forever.
  await db
  ...
  await db
    ...
    WHERE attachment_id IN (SELECT j.attachment_id FROM receipt_ocr_jobs j
      ...
      WHERE j.status IN ('done','needs_review','cancelled') AND COALESCE(j.reader_revision,0)<?
  ...
  try {
    ...
    const reviewReason =
      amount === null
        ...
        : details.blocked || details.ambiguous
