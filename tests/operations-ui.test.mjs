...
test('editor confirmado: preserva custo omitido e pagamentos; salva status na mesma operação',async()=>{
 ...
 const context={forgetWorking:()=>{},trackedItem:()=>false,working,workingDirty:true,can:p=>p!=='costs.view',paymentEditable:()=>false,cents,editorNumbers:()=>({diff:0}),api:async(path,data,method)=>{calls.push({path,data,method});return{id:'sale'};},load:async()=>{},render:()=>{},toast:()=>{},money,view:'editor',detailId:null,detailSale:null};
 context.finishMutation=async()=>{await context.load();context.detailSale=await context.api('/sales/sale');context.render();};
 ...
 context.working=working;context.workingDirty=true;context.view='editor';context.api=async()=>{throw Error('Atualize antes de editar');};await assert.rejects(()=>runInNewContext(`${source}\nsaveWorking(false)`,context),/Atualize/);assert.equal(context.workingDirty,true);assert.equal(context.view,'editor');
});
...
test('editor completo: clique no produto busca venda atual e foca o nome; nova data do pagamento fica fixa',async()=>{
 ...
 const calls=[],latest={id:'sale',edit_token:'fresh',items:[{id:'i'}]},context={rememberWorking:()=>{},pendingWorks:new Map(),a:'edit-sale',key:'sale',button:{dataset:{itemId:'i'}},load:async()=>calls.push('load'),api:async path=>{calls.push(path);return latest;},makeWorking:s=>s,render:()=>calls.push('render'),document:{querySelector:selector=>({focus:()=>calls.push(selector)})}};
