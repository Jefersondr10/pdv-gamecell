...
test('identificação: trocar produto não herda IMEI ou divulgação do aparelho anterior',t=>{
 ...
 x.db.saveDraft(x.actor, {draft_token:x.db.operations.draftToken(x.actor,id),...({items:[{...payload([item])[0],product_id:p2.id}]})}, id);item=x.db.sale(x.actor,id).items[0];
 assert.equal(item.serial_number,'');assert.equal(item.details,'');assert.equal(item.share_details,false);
 x.db.saveDraft(x.actor, {draft_token:x.db.operations.draftToken(x.actor,id),...({items:[{...payload([item])[0],serial_number:'NOVO',share_details:true}]})}, id);item=x.db.sale(x.actor,id).items[0];
 x.db.saveDraft(x.actor, {draft_token:x.db.operations.draftToken(x.actor,id),...({items:[{...payload([item])[0],product_id:null,description:'Avulso'}]})}, id);
...
test('identificação: catálogo e avulso guardam texto sem afetar custo ou estoque',t=>{
 ...
 assert.throws(()=>x.db.saveDraft(x.actor, {draft_token:x.db.operations.draftToken(x.actor,id),...({items:[]})}, id),/confirmada/);
});
test('identificação: editar preserva omitidos, reordena e limpa valores explicitamente',t=>{
 ...
 x.db.saveDraft(x.actor, {draft_token:x.db.operations.draftToken(x.actor,id),...({items})}, id);sale=x.db.sale(x.actor,id);
 assert.deepEqual(sale.items.map(i=>i.serial_number),['SN2','SN1']);assert.equal(sale.items[1].share_details,true);
 x.db.saveDraft(x.actor, {draft_token:x.db.operations.draftToken(x.actor,id),...({items:[{...payload(sale.items)[1],serial_number:null,details:'',share_details:false}]})}, id);
...
test('identificação: limites, tipos, IDs repetidos e dados de outra venda são rejeitados atomicamente',t=>{
 ...
 for(const extra of [{serial_number:123},{details:{}},{serial_number:'a'.repeat(1001)},{details:'a'.repeat(2001)},{share_details:'true'}])assert.throws(()=>x.db.saveDraft(x.actor, {draft_token:x.db.operations.draftToken(x.actor,id),...({items:[{...plain({id:item.id}),...extra}]})}, id));
 assert.throws(()=>x.db.saveDraft(x.actor, {draft_token:x.db.operations.draftToken(x.actor,id),...({items:[plain({id:item.id}),plain({id:item.id})]})}, id),/repita/);
 const second=save(x,[plain()]);assert.throws(()=>x.db.saveDraft(x.actor, {draft_token:x.db.operations.draftToken(x.actor,second),...({items:[plain({id:item.id})]})}, second),/nesta venda/);
...
test('identificação: permissões de venda não dependem de consultar ou informar custo',t=>{
 ...
 x.db.saveDraft(restricted, {draft_token:x.db.operations.draftToken(restricted,id),...({items:[{...payload([item])[0],details:'Conferido'}]})}, id);
...
test('identificação: migração aditiva preserva linhas antigas e persiste após reabertura',t=>{
 ...
 const item=db.sale(x.actor,id).items[0];db.saveDraft(x.actor, {draft_token:db.operations.draftToken(x.actor,id),...({items:[{...payload([item])[0],serial_number:'SN-PERSISTE'}]})}, id);db.close();
