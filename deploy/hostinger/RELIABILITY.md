...
3. Build both images with the label `org.opencontainers.image.revision=FULL_COMMIT_SHA`. Execute `sh scripts/hostinger/deploy-reliability.sh TAG EXPECTED_OLD_TAG /opt/atacadoapple/release-TAG FULL_COMMIT_SHA`. It checks source labels and the exact live image/mount, confirms a fresh off-site backup, saves rollback configuration, runs only additive migration 0009 (with an online pre-migration database copy), starts/checks OCR, then switches/checks the app. Unexpected configuration failures also restore the previous configuration/app. Never reuse the old ranking-only deployment script.
